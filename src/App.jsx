...
function App() {
  ...
  useEffect(() => {
    console.log("AI Config Status:", aiApiKey ? "Loaded Successfully" : "MISSING Key from .env");
  }, [aiApiKey]);
  ...
  const generateAIInsights = async () => {
    if (!aiApiKey || aiApiKey === "your_api_key_here") {
      alert("Missing Gemini API Key. Please add it to the .env file.");
      return;
    }
    ...
    try {
      ...
      alert(`Error with ${selectedModel}: ${err.message}. Try another model or check 'Discover Models' in console.`);
    } finally {
          ...
          <section className="bg-slate-900 rounded-[2.5rem] p-10 overflow-hidden relative shadow-2xl">
            ...
            <div className="relative flex flex-col xl:flex-row gap-12">
               <div className="xl:w-1/3">
                  ...
                  <p className="text-slate-400 text-lg leading-relaxed mb-8">Empower strategy using the .env GEMINI_API_KEY. Enterprise-grade reasoning.</p>
                  ...
                  {(!aiApiKey || aiApiKey === "your_api_key_here") && (
                    <p className="mt-4 text-xs font-bold text-red-400 flex items-center gap-2">
                       <Lock className="h-3 w-3" /> API Key Missing in .env
                    </p>
                  )}
               </div>
